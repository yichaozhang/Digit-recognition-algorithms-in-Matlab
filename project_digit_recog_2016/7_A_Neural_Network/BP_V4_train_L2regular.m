...
%%% Date:           2015-09-04
%%% Description:
  % In this function, we add regularization on cost function, which is set as: 
  % C = -1/n*sum(ylna+(1-y)ln(1-a))+lamda/2n*sum(weight^2) (cross-
  % entropy cost), and initially randomly select the values of weights and bias.
%%% Input
...
function [cost_value_average,BP_net,test_info] = BP_V4_train_L2regular(digit_dimension,BP_parameter,lamda)

global main_folder  
global BP_net_V4_str trn_data_num test_data_num 

file_to_open_BP_net = [main_folder,'49_data\',BP_net_V4_str];
...
for ii = 1:data_selec_per_time                                              % BP iteration begins
  ...
  %%% 2.5 Compute the output error vector delta for the output layer
    wei_hidden_output_update = wei_hidden_output_update+(output_activation-label_input_1)*hidden_activation';
    bias_hidden_output_update = bias_hidden_output_update+(output_activation-label_input_1);
    
  %%% 2.6 Backpropagate the error: Compute the error vector dalta for the hidden layer
    delta_x_hidden = (wei_hidden_output'*(output_activation-label_input_1)).*diff_hidden_value;     % (number of hidden neurons)*1
  ...
  %%% 2.7 Calculate the cost value of this train data, and add it to the total cost for this set of train data. 
    % Cost function in this function: 1/(n)*sum(y_x*ln(a_x)+(1-y_x)*ln(1-a_x))
    cost_value_1 = -1* sum( label_input_1.*log(output_activation)+(1-label_input_1).*log(1-output_activation) )+lamda/(2*trn_data_num)*sum(sum(wei_hidden_output_update.^2));
...
wei_input_hidden  = (1-lamda*learn_rate/trn_data_num)*wei_input_hidden-learn_rate/data_selec_per_time*wei_input_hidden_update;
wei_hidden_output = (1-lamda*learn_rate/trn_data_num)*wei_hidden_output-learn_rate/data_selec_per_time*wei_hidden_output_update;
...
file_to_save = [main_folder,'49_data\',BP_net_V4_str];
