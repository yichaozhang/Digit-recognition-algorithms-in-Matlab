...
%%% Date:           2015-09-14
%%% Description:
  % A non-linear classification technique based on Fisher's discriminant is
  % proposed. The main ingredient is the kernel trick which allows the
  % efficient computation of Fisher discriminant in feature space. The
  % linear classification in feature space corresponds to a (powerful) non-
  % linear decision function in input space.
%%% Input
...
%%% Notice, Kernel Fisher Discriminant shows great performance when the number 
  % of training data is small. When it is big, then running speed is too slow.  
  
%%% More detail: 'Fisher Discriminant Analysis with Kernels'

function [accuracy,label_estimate] = fisher_kernel (digit_dimension,sigma)

global main_folder trn_data_num_small test_data_num 
...
data_trn     = Trainnumbers.trn_image_ex(:,1:trn_data_num_small);           % Extract training dataset (784*trn_data_num)
label_trn    = Trainnumbers.trn_label_ex(1:trn_data_num_small,:)';          % Extract training labels (1*trn_data_num)
image        = 0;
...
for class1 = 2:10
    for class2 = 1:(class1-1)
        %%% 2nd step: Fisher Kernel Discriminant classifier
        class2
        classfit = fisher_kernel_classify(class1,class2,test_af_PCA,sigma);
%%% INPUT
